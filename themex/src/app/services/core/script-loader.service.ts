...
export class ScriptLoaderService {
  ...
  loadPixelScript(pixelId: string): void {
    ...
    script.onload = () => {
      (window as any).fbq('init', pixelId);                 // ✅ শুধু init
      (window as any).fbq('set', 'autoConfig', false, pixelId);      // ✅ আলাদা করে সেট
      (window as any).fbq('set', 'allowAutomaticEvents', false);     // ✅ auto events বন্ধ
      console.log('FB Pixel loaded', (window as any).fbq?.version);
    };

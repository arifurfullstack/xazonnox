...
export class Checkout2Component implements OnInit, AfterViewInit, OnDestroy {
  ...
  onChangeAddress(event: UserAddress): void {
    ...
    const isShippingAddress =
      typeof event?.shippingAddress === 'string' && event.shippingAddress.trim().length > 0;
    const isShippingAddressString = this.shippingAddress !== event.shippingAddress;


    if (isIncompleteOrderEnabled && isPhoneValid && (isPhoneChanged || isShippingAddressString) && isShippingAddress && !this.isIncompleteOrderId) {
      ...
      if (isIncompleteOrderEnabled && isPhoneValid && (isPhoneChanged || isShippingAddressString) && isShippingAddress &&  this.incompleteOrderId && this.isIncompleteOrderId) {
        this.phoneNo = event.phoneNo;
        this.shippingAddressString = event.shippingAddress;

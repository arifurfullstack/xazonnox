...
export class SuccessOrderComponent implements OnInit, OnDestroy {
  ...
  /**
   * Process success page message by replacing placeholders with actual values
   */
  private processSuccessPageMessage(): string {
    if (!this.successPageMessage || !this.orderId) {
      return this.successPageMessage || '';
    }

    // Replace {{orderId}} placeholder with actual order ID
    return this.successPageMessage.replace(/\{\{orderId\}\}/g, this.orderId);
  }

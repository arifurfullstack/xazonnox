...
@Component({
  selector: 'app-product-details-banner-2',
  templateUrl: './product-details-banner-2.component.html',
  styleUrls: ['./product-details-banner-2.component.scss'],
  imports: [
...
export class ProductDetailsBanner2Component
  implements OnInit, OnChanges, OnDestroy
{
  ...
  messengerLink: string | null = null;
  ...
  ngOnInit() {
    ...
    // Load shop contact info
    const sub3 = this.shopInformationService.getShopInformation().subscribe({
      next: (res) => {
        const phones = res?.data?.phones;
        const socialLinks = res?.data?.socialLinks;
        const whatsappNumber = (res?.data as any)?.whatsappNumber;
        this.phoneNo = phones && phones.length ? phones[0]?.value : null;
        const normalizedPhone = (whatsappNumber || this.phoneNo || '').replace(
          /[^0-9+]/g,
          ''
        );
        this.whatsappLink = normalizedPhone
          ? `https://wa.me/${normalizedPhone.replace(/^\+/, '')}`
          : null;
        const fb = Array.isArray(socialLinks)
          ? socialLinks.find((s: any) => s?.type === 0)?.value
          : null;
        if (fb && typeof fb === 'string') {
          // Try convert facebook page/profile link to messenger link if possible
          // e.g., https://facebook.com/xyz -> https://m.me/xyz
          const match = fb.match(/facebook\.com\/([^/?#]+)/i);
          this.messengerLink =
            match && match[1] ? `https://m.me/${match[1]}` : null;
        }
      },
      error: () => {},
    });
  ...
  onSelectVariation(name: string) {
    console.log('locked on select variation', name);
    if (this.selectedVariation !== name) {
      this.selectedVariation = name;
      console.log('locked on ----- variation', this.selectedVariation);
      if (
    ...
    if (this.selectedVariation2 !== name) {
      ...
      let image = this.product?.variationList.find(
        (v) =>
          ...
          v?.name.toLowerCase().indexOf(this.selectedVariation.toLowerCase()) >
            -1
      ).image;
      this.image = image && image.length > 0 ? image : this.prevImage;
      this.setSelectedVariationList();
    ...
    if (this.showWholesale && this.product?.minimumWholesaleQuantity > 0) {
      if (this.selectedQty === this.product?.maximumWholesaleQuantity) {
        this.uiService.message(
          `Maximum quantity are ${this.product?.maximumWholesaleQuantity}`,
          'warn'
        );
        return;
      }
  ...
  onDecrementQtySimple(event: MouseEvent) {
    ...
    if (this.showWholesale && this.product?.minimumWholesaleQuantity > 0) {
      if (this.selectedQty === this.product?.minimumWholesaleQuantity) {
        this.uiService.message(
          `Minimum quantity is ${this.product?.minimumWholesaleQuantity}`,
          'warn'
        );
        return;
      }
  ...
  onAddToCart(event: MouseEvent, type: 'addToCart' | 'buyNow') {
    ...
    if (this.userService.isUser) {
      ...
      if (
        this.product?.isVariation &&
        this.selectedVariationList?.name == null
      ) {
  ...
  private checkWishlist() {
    this.wishlist = this.wishlists.find(
  ...
  scrollBody() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      const footerOffsetTop =
        document.getElementById('footerSection')?.offsetTop || 0;
  ...
  private addToWishlistDB(data: Wishlist, url?: string) {
    const subscription = this.wishlistService.addToWishlist(data).subscribe({
      next: (res) => {
        this.uiService.actionMessage(
          res?.message,
          'success',
  ...
  updateTransform() {
    this.transformStyle = `translateX(-${

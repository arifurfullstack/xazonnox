...
export class ProductCategoriesComponent implements OnInit, OnDestroy {
  ...
  private getSettingData(): void {
    ...
    this.productCardViews =
      themeViewSettings
        ...
        ?.value.join() || '';
    this.categoryCardViews =
      themeViewSettings.find((f) => f.type === 'categoryViews')?.value.join() ||
      '';
  }
  ...
  private getAllProducts(loadMore?: boolean): void {
    ...
    const filterData: FilterData = {
      ...
      filterGroup: loadMore
        ? null
        : { isGroup: true, category: true, subCategory: true, brand: true },
      select: {
        name: 1,
        isVariation: 1,
        images: 1,
    ...
    const subscription = this.productService
      ...
      .subscribe({
        next: (res) => {
          ...
          this.products = loadMore ? [...this.products, ...pageData] : pageData;
          this.totalProducts = res.count ?? this.totalProducts;
          if (!loadMore && !this.productFilterGroup)
  ...
  private getAllSeoPage() {
    const subscription = this.seoPageService
      .getAllSeoPageByUi({ status: 'publish', type: 'category-page' }, 1, 1)
      .subscribe({
        next: (res) => {

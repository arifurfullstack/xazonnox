...
import {IpBlockService} from "../../../services/common/ip-block.service";

@Component({
  selector: 'app-all-ip-block',
  templateUrl: './all-ip-block.component.html',
  styleUrl: './all-ip-block.component.scss',
  providers: [DatePipe]
})
export class AllIpBlockComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnInit, OnDestroy {
  ...
  private readonly ipBlockService = inject(IpBlockService);
  ...
  private setPageData(): void {
    this.title.setTitle('IP Block');
    this.pageDataService.setPageData({
      title: 'IP Block',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'IP Block', url: null},
      ]
    })
  }
  ...
  private getAllIpBlock() {
    ...
    const subscription = this.ipBlockService.getAllIpBlock(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleCategoryById() {
    const subscription = this.ipBlockService.deleteMultipleIpBlockById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    const subscription = this.ipBlockService.updateMultipleIpBlockById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
